...
void trans(char * X)
{
  ...
  int flag_a, flag_b; //flags to get the 0,1,2,3...etc.
  for (flag_a=0;flag_a<image_num;flag_a++)//for loop to get the correspoding name for each images
    {
      //compare the flag image to the all images' name that has stored and get the correct one
...
void pri(char * X)//print to stdout
{
  ...
  for (flag_a=0;flag_a<image_num;flag_a++)//for loop to get the correspoding name for each images
	...
	{
	  for (flag_b=0;flag_b<(i_name[flag_a].size+1);flag_b++)//for loop to get the correspoding 
	    {
