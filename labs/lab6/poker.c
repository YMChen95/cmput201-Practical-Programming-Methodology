...
bool straight, flush, four, three, royal,straight1;
...
void analyze_hand(int num_in_rank[NUM_RANKS], int num_in_suit[NUM_SUITS])
{
  ...
  for (; rank < NUM_RANKS && num_in_rank[rank] > 0; rank++)
    num_consec++;
  if (card_exists[12]&&card_exists[0]&&card_exists[1]&&card_exists[2]&&card_exists[3]){
    straight1 = true;
  }
...
void print_result(void)
{
  ...
  else if (straight1 && flush) printf("Straight flush");
  else if (four)         printf("Four of a kind");
  else if (three &&
           pairs == 1)   printf("Full house");
  else if (flush)        printf("Flush");
  else if (straight||straight1)     printf("Straight ");
